...
export const BrowserPreview: React.FC<BrowserPreviewProps> = ({ currentStep, screenshotUrl, isProcessing }) => {
  ...
  return (
    <GlassPane title="Remote Browser" className="h-full relative group">
      ...
      <div className="bg-white/80 border-b border-slate-200 p-3 flex items-center gap-3">
        ...
        {/* URL Bar */}
        <div className="bg-slate-100/80 border border-slate-200 rounded-md px-4 py-1.5 text-xs text-slate-500 font-mono flex-1 text-center truncate flex items-center justify-center gap-2 shadow-inner">
           ...
           {currentStep?.target || "app.gohighlevel.com"}
        </div>
      ...
      <div className="w-full h-full bg-slate-50 relative flex items-center justify-center overflow-hidden">
        ...
        {isBuilding && isProcessing && (
          <div className="absolute inset-0 z-10 pointer-events-none border-4 border-emerald-500/30 border-dashed animate-pulse">
            <div className="absolute bottom-4 right-4 bg-emerald-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg flex items-center gap-2">
